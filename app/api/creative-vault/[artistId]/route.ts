...
export async function GET(request: Request, context: NextRequest['context'] & { params: { artistId: string } }) {
...
export async function POST(request: Request, context: NextRequest['context'] & { params: { artistId: string } }) {
  const { artistId } = context.params;
  const cookieStore = cookies();
  const supabase = await createClient(cookieStore);
...
export async function PUT(request: Request, context: NextRequest['context'] & { params: { artistId: string } }) {
  const { artistId } = context.params;
...
export async function DELETE(request: Request, context: NextRequest['context'] & { params: { artistId: string } }) {
  const { artistId } = context.params;
  const { searchParams } = new URL(request.url);
  const itemId = searchParams.get('itemId');
