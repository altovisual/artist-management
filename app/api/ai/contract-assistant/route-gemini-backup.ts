...
// --- Model Definition ---
// TEMPORARY FIX: Using basic model without tools due to API version compatibility
const model = genAI.getGenerativeModel({
  model: "gemini-1.5-flash-latest",
  systemInstruction: systemPrompt,
  // Commenting out tools temporarily due to v1beta API limitations
  /* tools: [{
...
export async function POST(request: Request) {
  try {
    ...
    // TEMPORARY: Disabled function calling due to API compatibility
    const functionCalls = null; // response.functionCalls();

    if (false && functionCalls && functionCalls.length > 0) {
